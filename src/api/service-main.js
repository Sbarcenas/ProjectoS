...
export default class ServiceMain {
	...
	constructor(serviceName, watch = false, transforms = []) {
		this.serviceName = serviceName.trim();
		this.service = feathersClient.service(this.serviceName);
		this.service.timeout = TIMEOUT;

		const listeningHandler = type => (data, context) => ServiceMain.event.next({type, serviceName, data, context});
		...
		if (typeof watch === "boolean" && watch) {
			...
			if (watch.created) this.service.on("created", listeningHandler(ServiceMain.TYPE_CREATED));
			if (watch.updated) this.service.on("updated", listeningHandler(ServiceMain.TYPE_UPDATED));
			if (watch.patched) this.service.on("patched", listeningHandler(ServiceMain.TYPE_PATCHED));
			if (watch.removed) this.service.on("removed", listeningHandler(ServiceMain.TYPE_REMOVED));
		}
	}

	static getEvent(serviceName, type) {
		return ServiceMain.event.pipe(filter(it => it.serviceName === serviceName && it.type === type));
	}

	static getEventByService(serviceName) {
		return ServiceMain.event.pipe(filter(it => it.serviceName === serviceName));
	}
	...
	getEvent(type = [ServiceMain.TYPE_CREATED, ServiceMain.TYPE_UPDATED, ServiceMain.TYPE_PATCHED, ServiceMain.TYPE_REMOVED]) {
